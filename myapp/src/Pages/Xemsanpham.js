...
const ViewProduct = () => {
  ...
  if (productDetails.MucGiamGia == null || productDetails.MucGiamGia === 0)
    ...
    pricePerProduct =
      productDetails.Gia * (1 - productDetails.MucGiamGia / 100);
      ...
      <div className={styles.view_product}>
        <div className={styles.view_product_top}>
          ...
          <div className={styles.column_2}>
            <h2>{productDetails.Ten}</h2>
            <div>
              <p>
                Nhà sản xuất:{" "}
                <span className={styles.author}>
                  {productDetails.NhaSanXuat}
                </span>
              </p>
              <p>
                Xuất xứ: <span className={styles.origin}>Việt Nam</span>
              </p>
              ...
              {productDetails.MucGiamGia == null ||
              productDetails.MucGiamGia === 0 ? (
                <div className={styles.moneyContainer}>
                  <p className={styles.moneyText_1}>{productDetails.Gia} đ</p>
                </div>
              ) : (
                <div className={styles.moneyContainer}>
                  <p className={styles.moneyText_1}>
                    {(
                      productDetails.Gia *
                      (1 - productDetails.MucGiamGia / 100)
              ...
              <div className={styles.infoContainer}>
                ...
                <div>
                  ...
                  <span className={styles.origin}>
                    {productDetails.SoLuongDaBan}
                  </span>{" "}
                </div>
                <div>
                  Hỗ trợ trả hàng:{" "}
                  <span className={styles.origin}>
                    {productDetails.HoTroTraHang}
                  </span>{" "}
                </div>
              </div>
              <div className={styles.infoContainer}>
                ...
                <button
                  ...
                  className={styles.info_button_1}
                >
                  -
                </button>
              ...
              <>
                <button
                  className={styles.view_button_1}
                  onClick={() => addToCart(bookId, count, userInfo)}
                >
            ...
            <div className={styles.commentTop}>
              <div>
                ...
                {reviewDetails && reviewDetails.length > 0 ? (
                  <>
                    <p>
                      <span className={styles.commentStar1}>
                        {productDetails.DiemTrungBinh.toFixed(1)}
                      </span>
                    ...
                    {reviewDetails.map((review, index) => (
                      <React.Fragment key={index}>
                        <div className={styles.setNameComment}>
                          {review.HoTen} {review.SoSao}{" "}
                          <MdStar className={styles.colorStar} />
                        </div>
                        <div className={styles.describeStory2}>
                          <p> {review.MoTa}</p>
                        </div>
                        <hr className={styles.lineSeperate}></hr>
                      </React.Fragment>
